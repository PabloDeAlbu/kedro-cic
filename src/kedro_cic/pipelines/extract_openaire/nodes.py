...
def refresh_access_token(refresh_token, timeout=30):
    ...
    response = requests.get(refresh_url, timeout=timeout)
...
def get_with_refresh(
    ...
    while True:
        ...
        new_token = refresh_access_token(refresh_token, timeout=timeout)
        if not new_token:
...
def extract_openaire_researchproduct(filter_param, ror_filter_value, access_token, refresh_token, env):
    cursor = '*'
    ...
    print(f"OpenAIRE extract env={env}, page_size={page_size}, iteration_limit={iteration_limit}")
    ...
    while True:
        ...
        while cursor:
            if env == "dev" and iteration_count >= iteration_limit:
                print(f"Stopping pagination because env=dev reached iteration_limit={iteration_limit}.")
                break
